...
class VoiceHandler {
    ...
    async sendVoiceUpdate(node, guildId, state) {
    ...
    async connect(player, channel, sendGatewayPayload) {
    ...
    handleRawEvent(packet, userId, playerStateManager, nodeManager) {
        if (!packet || typeof packet !== "object")
            return;
        const t = packet.t;
        if (!t || (t !== "VOICE_STATE_UPDATE" && t !== "VOICE_SERVER_UPDATE"))
            return;
        const data = packet.d;
        ...
        const voiceStateChanged = prevVoiceState?.sessionId !== state.voiceState?.sessionId ||
        ...
        if (voiceStateChanged &&
            ...
            this.sendVoiceUpdate(state.node, guildId, state).catch((error) => this.debug(`Failed to send voiceUpdate for ${guildId} to ${state.node.identifier}`, error));
