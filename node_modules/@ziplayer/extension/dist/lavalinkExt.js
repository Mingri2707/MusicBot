...
exports.lavalinkExt = void 0;
const ziplayer_1 = require("ziplayer");
// Import modules
const NodeManager_1 = require("./managers/NodeManager");
const PlayerStateManager_1 = require("./managers/PlayerStateManager");
 ...
 * @since 1.0.0
 */
class lavalinkExt extends ziplayer_1.BaseExtension {
    /**
    ...
    constructor(player = null, opts) {
        super();
        this.name = "lavalinkExt";
        this.version = "1.0.0";
        this.player = null;
        this.originalMethods = new WeakMap();
        this.isReady = false;
        ...
        this.debug = (0, helpers_1.createDebugLogger)(this.options.debug ?? false, "lavalinkExt");
        // Initialize managers
        this.nodeManager = new NodeManager_1.NodeManager(this.options);
        this.playerStateManager = new PlayerStateManager_1.PlayerStateManager(this.options.debug ?? false);
        this.trackResolver = new TrackResolver_1.TrackResolver(this.options.debug ?? false);
        this.voiceHandler = new VoiceHandler_1.VoiceHandler(this.options.debug ?? false);
    ...
    async active(alas) {
        if (alas?.manager && !this.manager) {
            this.manager = alas.manager;
        }
        const providedClient = alas?.client;
        if (providedClient && !this.client) {
            this.client = providedClient;
            this.bindClient(providedClient);
        }
        const player = alas?.player || this.player;
        if (player) {
    ...
    async onDestroy(context) {
        ...
        const cleanupPromises = Array.from(allStates.entries()).map(async ([player, state]) => {
            if (state.node) {
                try {
    ...
    bindClient(client) {
        ...
        client.on("raw", (packet) => this.voiceHandler.handleRawEvent(packet, this.userId, this.playerStateManager, this.nodeManager));
    }
    ...
    startUpdateLoop() {
        ...
        const interval = this.options.updateInterval ?? 30000; // 30 seconds instead of 5
    ...
    handleWebSocketPlayerUpdate(node, message) {
        ...
        this.debug(`WebSocket player update for guild ${message.guildId}: position=${message.state.position}, connected=${message.state.connected}`);
    }
    handleWebSocketTrackStart(node, message) {
        const player = this.playerStateManager.getPlayerByGuildId(message.guildId);
        if (!player)
            return;
        const state = this.playerStateManager.getState(player);
    ...
    handleWebSocketTrackEnd(node, message) {
        const player = this.playerStateManager.getPlayerByGuildId(message.guildId);
        if (!player)
            return;
        const state = this.playerStateManager.getState(player);
    ...
    handleWebSocketTrackException(node, message) {
        const player = this.playerStateManager.getPlayerByGuildId(message.guildId);
        if (!player)
            return;
        const state = this.playerStateManager.getState(player);
    ...
    handleWebSocketTrackStuck(node, message) {
        const player = this.playerStateManager.getPlayerByGuildId(message.guildId);
        if (!player)
            return;
        const state = this.playerStateManager.getState(player);
    ...
    handleWebSocketClosed(node, message) {
        const player = this.playerStateManager.getPlayerByGuildId(message.guildId);
        if (!player)
            return;
        const state = this.playerStateManager.getState(player);
    ...
    async updatePlayerState(player, state) {
        if (!state.node || !state.node.wsConnected)
            return;
    ...
    attachToPlayer(player) {
        ...
        if (!this.manager) {
            const maybeManager = player.pluginManager?.manager ?? player?.manager;
            if (maybeManager)
                this.manager = maybeManager;
        }
        if (!this.originalMethods.has(player)) {
            ...
            player.skip = () => this.skipWithFallback(player);
            player.stop = () => this.stopWithFallback(player);
            player.pause = () => this.pauseWithFallback(player);
            player.resume = () => this.resumeWithFallback(player);
            player.setVolume = (volume) => this.setVolumeWithFallback(player, volume);
            player.connect = async (channel) => this.connect(player, channel);
        }
        const onDestroy = () => {
            this.destroyLavalinkPlayer(player).catch((error) => this.debug(`Failed to destroy Lavalink player for guild ${player.guildId}`, error));
    ...
    detachFromPlayer(player) {
        ...
        if (original) {
            player.skip = original.skip;
            player.stop = original.stop;
            player.pause = original.pause;
            player.resume = original.resume;
            player.setVolume = original.setVolume;
            player.connect = original.connect;
            this.originalMethods.delete(player);
        }
        const state = this.playerStateManager.getState(player);
        if (state) {
            this.destroyLavalinkPlayer(player).catch((error) => this.debug(`Failed to destroy Lavalink player during detach for ${player.guildId}`, error));
        }
        // Use PlayerStateManager
     ...
     * }
     */
    async beforePlay(context, payload) {
        const player = context.player;
        this.attachToPlayer(player);
        await this.initializeNodes();
        const requestedBy = payload.requestedBy ?? "Unknown";
        try {
            const { tracks, isPlaylist } = await this.trackResolver.resolvePlayRequest(player, payload.query, requestedBy, this.nodeManager, this.options.searchPrefix);
            if (tracks.length === 0) {
                return {
                    handled: false,
                    success: false,
     ...
     * }
     */
    async provideSearch(_context, payload) {
        try {
            return await this.trackResolver.searchLavalink(payload.query, payload.requestedBy, this.nodeManager, this.options.searchPrefix);
        }
        catch (error) {
            this.debug(`provideSearch error: ${error.message}`);
            return null;
        }
    }
    /**
     ...
     * }
     */
    async provideStream(_context, payload) {
        try {
            ...
            if ((0, helpers_1.getEncoded)(currentTrack) !== (0, helpers_1.getEncoded)(track)) {
            ...
            return {
                ...
                metadata: {
                    ...
                    encoded: (0, helpers_1.getEncoded)(track),
                },
            };
        }
        catch (error) {
            this.debug(`provideStream error: ${error.message}`);
            return null;
        }
    }
    async ensureNodeForPlayer(player) {
        ...
        const shouldSendVoiceUpdate = needsNewNode &&
    ...
    async connect(player, channel) {
        const original = this.originalMethods.get(player)?.connect;
        const channelId = channel?.id ?? channel ?? null;
        if (!channelId)
            throw new Error("Invalid channel provided to connect");
        ...
        if (!original)
            throw new Error("Player connect method missing");
    ...
    async startNextOnLavalink(player, ignoreLoop = false) {
        const node = await this.ensureNodeForPlayer(player);
        const state = this.playerStateManager.getState(player);
        if (!state)
            throw new Error("Missing state for player");
        ...
        player.clearLeaveTimeout?.();
        await player.generateWillNext?.();
        try {
            await this.playerStateManager.waitForVoice(player, this.options.requestTimeoutMs);
        }
        catch (error) {
        ...
        try {
            const isLavalinkTrack = track.source === "lavalink" || (0, helpers_1.getEncoded)(track);
            if (isLavalinkTrack) {
                ...
                const encoded = (0, helpers_1.getEncoded)(track);
                if (!encoded)
                    throw new Error("Track has no Lavalink payload");
                track.metadata = {
                    ...(track.metadata ?? {}),
                    lavalink: {
    ...
    async destroyLavalinkPlayer(player) {
        const state = this.playerStateManager.getState(player);
        if (!state?.node)
            return;
        try {
    ...
    pause(player) {
        const state = this.playerStateManager.getState(player);
        if (!state?.node || !state.playing || state.paused)
            return false;
    ...
    resume(player) {
        ...
        const track = state.track ?? player.queue.currentTrack ?? undefined;
        if (track)
            player.emit("playerResume", track);
        this.nodeManager
    ...
    stop(player) {
        ...
        this.destroyLavalinkPlayer(player).catch((error) => this.debug(`Failed to destroy Lavalink player during stop for ${player.guildId}`, error));
        return true;
    }
    async skip(player) {
        const state = this.playerStateManager.getState(player);
    ...
    setVolumeWithFallback(player, volume) {
        const state = this.playerStateManager.getState(player);
        if (state?.node) {
...
exports.lavalinkExt = lavalinkExt;
//# sourceMappingURL=lavalinkExt.js.map